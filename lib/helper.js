...
export function createRecipeDataObject(
  ...
  const steps = recipeStepsList.map((recipeStep) => ({
...
export function calculateShoppingList(
  ...
  pantry.forEach((item) => {
    if (!item.ingredient || !item.unit) return;
    const key = `${item.ingredient.name}-${item.unit.name}`;
    pantryMap.set(key, (pantryMap.get(key) ?? 0) + item.amount);
  });

  const result = [];
  merged.forEach((item, key) => {
    const pantryAmount = pantryMap.get(key) ?? 0;
    const remaining = item.amount - pantryAmount;
